...
function Client(options) {
  ...
  this._lastfrontBackDegrees = 0;
  this._lastrotationDegrees = 0;
  this._lastxVelocity	  = 0;
  this._lastyVelocity	  = 0;
  this._lastzVelocity	  = 0;
}
...
Client.prototype._processNavdata = function(navdata) {
  if (navdata.droneState && navdata.demo) {
   ...
   //xiaodong test events---------------------------------------------------------------------------
    var fbDegrees = navdata.demo.frontBackDegrees;
    if (fbDegrees !== this.lastfrontBackDegrees){
	//this.emit('fbchange',fbDegrees);
        this._lastfrontBackDegrees = fbDegrees;
    }

    // pitch event
    var pitchDegrees = navdata.demo.rotation.pitch;
    if (pitchDegrees !== this._lastrotationDegrees){
	//this.emit('fbchange',fbDegrees);
        this._lastrotationDegrees = pitchDegrees;
    }
    // var velocity = navdata.demo.velocity;
    var xVelocity = navdata.demo.xVelocity;
    var yVelocity = navdata.demo.yVelocity;
    var zVelocity = navdata.demo.zVelocity;
   // console.log('x= '+ xVelocity +' y= '+ yVelocity + ' z ' = zVelocity);
    if (xVelocity !== this._lastxVelocity){
        this._lastxVelocity = xVelocity;
    }	
    if (yVelocity !== this._lastyVelocity){
        this._lastyVelocity = yVelocity;
    }
     
    if (zVelocity !== this._lastzVelocity){
        this._lastzVelocity = zVelocity;
    }		
    //---------------------------------------------------------------------------------------------
  }
};
...
//-------------test -------------------------------------------------------------
Client.prototype.altitude = function() {
  this.once('navdata',function(){});
  return this._lastAltitude;
};

Client.prototype.pitchDegrees = function () {
  this.on('navdata',function(){
});
  return this._lastrotationDegrees;
};

Client.prototype.frontback = function () {
  this.once('navdata',function(){
});
 return this._lastfrontBackDegrees;
};
//-----------------Vx Vy Vz--------------------
Client.prototype.xVelocity = function (){
  this.once('navdata',function(){});
 return this._lastxVelocity;
};

Client.prototype.yVelocity = function (){
  this.once('navdata',function(){});
 return this._lastyVelocity;
};

Client.prototype.zVelocity = function (){
  this.once('navdata',function(){});
 return this._lastzVelocity;
};
